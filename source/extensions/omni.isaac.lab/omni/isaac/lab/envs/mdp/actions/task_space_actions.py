...
from omni.isaac.lab.controllers import OperationSpaceController
...
class OperationSpaceAction(ActionTerm):
    r"""Inverse Kinematics action term.
    ...
    cfg: actions_cfg.OperationSpaceActionCfg
    """The configuration of the action term."""
    _asset: Articulation
    """The articulation asset on which the action term is applied."""
    _scale: torch.Tensor
    ...
    def __init__(self, cfg: actions_cfg.OperationSpaceActionCfg, env: ManagerBasedEnv):
        # initialize the action term
        super().__init__(cfg, env)
        ...
        self._operation_space_controller = OperationSpaceController(
            cfg=self.cfg.controller, num_robots=self.num_envs, num_dof=self._num_joints, device=self.device)
    ...
    @property
    def action_dim(self) -> int:
        return self._operation_space_controller.num_actions
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        self._operation_space_controller.set_command(self._processed_actions)
    ...
    def apply_actions(self):
        ...
        if ee_quat_curr.norm() != 0:
            jacobian = self._compute_frame_jacobian()
            joint_pos_des = self._operation_space_controller.compute(ee_pos_curr, ee_quat_curr, jacobian, joint_pos)
        else:
            joint_pos_des = joint_pos.clone()
        # set the joint position command
        self._asset.set_joint_position_target(joint_pos_des, self._joint_ids)
